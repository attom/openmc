module clustering
  ...
  use ace_header,        only: Nuclide, RrrData
...
!===============================================================================
  ...
  subroutine cluster_one_nuclide(i_nuclide)
    ...
    observations(1, :) = log(nuc % total(rrr%i_low : rrr%i_high) + SMALL)
    observations(2, :) = log(nuc % elastic(rrr%i_low : rrr%i_high) + SMALL)
    if (nuc % fissionable) then
      observations(3, :) = log(nuc % fission(rrr%i_low : rrr%i_high) + SMALL)
    end if
    !
    write (*,*) minval(observations), maxval(observations)

    ! Perform k-means clustering
    call kms_uniform_clust_cen(observations, n_clust_glob, clust_cen)
    ...
    do i = 1,size(clust_cen, 2)
      write(*, '(100f7.2)') (clust_cen(j, i), j=1,n_rxn)
    end do
    write (*,*) "Error is ", rrr % L2_err, " and ", rrr % Linf_err
